...
public class Pong extends Application
{
    ...
    public void update(long pretekliCas) throws IOException 
    {
        ...
        if(yz+h/108>=yp && xz>=xp && xz<=xp+w/15) //zoga se odbije od vrha loparja
        {
            smery=false;
            tocke++;
            if(tocke<=rekord)
    ...
    public void start(Stage mainStage) throws IOException
    {
        ...
        {
            @Override
            public void handle(long trenutniCas) 
            {
                if(!igraTece)
                {
                    prejsnjiCas=trenutniCas;
                    // System.out.println(prejsnjiCas);
                    return;
                }

                long pretekliCas = trenutniCas - prejsnjiCas;
                prejsnjiCas=trenutniCas;

                try 
                {
                    update(pretekliCas);
                    // System.out.println(pretekliCas);
                } 
                catch (IOException e) 
                {
                    e.printStackTrace();
    ...
    public Scene ustvariIgro()
    {
        ...
        {//zazna premikanje miske in s tem premika ploscek
            xp=event.getScreenX();
            ploscek.setX(xp);
            if (xp<0) //da ne gre ploscek cez levi rob
            {
                xp=0;
                ploscek.setX(xp);
            }
            if (xp>w-w/15) //da ne gre ploscek cez desni rob
            {
                xp=w-w/15;
                ploscek.setX(xp);
            }
